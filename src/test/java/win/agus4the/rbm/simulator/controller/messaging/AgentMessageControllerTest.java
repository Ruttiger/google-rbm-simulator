...
class AgentMessageControllerTest {
    ...
    @Test
    void triggersEventsSequentially() {
        ...
        Message message = new Message(
                null,
                null,
                new Message.AgentContentMessage("#DELIVERED(delay=20)#READ(delay=30)", null, null, null, null)
        );

        controller.receiveMessage("12345", "AGENT", "1", message).block();

        ArgumentCaptor<Map<String, Object>> captor = ArgumentCaptor.forClass(Map.class);
        verify(dispatcherService, timeout(200).times(2))
                .dispatchEvent(eq("AGENT"), captor.capture());

        List<Map<String, Object>> events = captor.getAllValues();
        assertThat(events).extracting(e -> e.get("eventType")).containsExactly("DELIVERED", "READ");
        assertThat(events).extracting(e -> e.get("messageId")).containsExactly("1", "1");
    }

    @Test
    void triggersMultipleIsTypingWithDelays() {
        when(messagingService.saveAgentMessage(anyString(), anyString(), any()))
                .thenReturn(Mono.empty());
        when(dispatcherService.dispatchEvent(anyString(), any()))
                .thenReturn(Mono.empty());
        ...
        Message message = new Message(
                null,
                null,
                new Message.AgentContentMessage("#IS_TYPING(delay=10)#IS_TYPING(delay=20)", null, null, null, null)
        );
        ...
        List<Map<String, Object>> events = captor.getAllValues();
        assertThat(events).extracting(e -> e.get("eventType")).containsExactly("IS_TYPING", "IS_TYPING");
        assertThat(events).allMatch(e -> !e.containsKey("messageId"));
    }
}
